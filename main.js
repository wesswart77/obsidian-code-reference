...
var VIEW_TYPE_CODE_REF = "code-reference-sidebar";
var CodeRefView = class extends import_obsidian.ItemView {
  ...
  async renderCards(container) {
    var _a, _b, _c, _d, _e;
    container.empty();
    const folder = this.activeTab === "snippets" ? this.plugin.settings.snippetsFolder : this.plugin.settings.patternsFolder;
    ...
    for (const file of files) {
      ...
      const lang = (_c = (_b = fm["language"]) != null ? _b : fm["languages"]) != null ? _c : "";
      const fw = (_d = fm["framework"]) != null ? _d : "";
      const desc = (_e = fm["description"]) != null ? _e : "";
      if (langFilter && !lang.toLowerCase().includes(langFilter))
        continue;
    ...
    for (const { file, lang, fw, desc } of cards) {
      ...
      if (lang)
        meta.createSpan({ cls: "crl-badge crl-badge-lang", text: lang });
      if (fw)
        meta.createSpan({ cls: "crl-badge", text: fw });
      if (desc)
        card.createDiv({ cls: "crl-card-desc", text: desc });
    }
  }
};
var SnippetModal = class extends import_obsidian.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian.Setting(contentEl).setName("Language").addDropdown((d) => {
      LANGUAGES.forEach((l) => d.addOption(l, l));
      d.setValue(this.language);
      d.onChange((v) => this.language = v);
    });
    new import_obsidian.Setting(contentEl).setName("Framework / Library (optional)").addText((t) => {
      t.setPlaceholder("e.g. React, Lodash").onChange((v) => this.framework = v);
    });
    new import_obsidian.Setting(contentEl).setName("Tags (comma-separated)").addText((t) => {
    ...
    new import_obsidian.Setting(contentEl).setName("Code").addTextArea((t) => {
      t.inputEl.addClass("crl-textarea");
      t.inputEl.rows = 8;
      t.setPlaceholder("Paste your code here...").onChange((v) => this.code = v);
    });
    new import_obsidian.Setting(contentEl).addButton(
      (b) => b.setButtonText("Save Snippet").setCta().onClick(() => this.save())
    );
  }
  async save() {
    var _a;
    ...
    const tagList = this.tags.split(",").map((t) => t.trim()).filter(Boolean).map((t) => `"${t}"`).join(", ");
    const highlight = (_a = LANG_HIGHLIGHT[this.language]) != null ? _a : "";
    ...
    if (this.app.vault.getAbstractFileByPath(path)) {
      new import_obsidian.Notice("A snippet with that name already exists.");
      return;
    }
    await this.app.vault.create(path, lines.join("\n"));
    new import_obsidian.Notice(`Snippet "${this.title}" saved.`);
    this.close();
    this.onSave();
  }
  onClose() {
...
var PatternModal = class extends import_obsidian.Modal {
  constructor(app, plugin, onSave) {
    super(app);
    this.title = "";
    this.category = "design-pattern";
    this.languages = "";
    this.description = "";
    this.example = "";
    this.plugin = plugin;
  ...
  onOpen() {
    ...
    new import_obsidian.Setting(contentEl).setName("Applicable Languages (comma-separated)").addText((t) => {
    ...
    new import_obsidian.Setting(contentEl).setName("Example Code").addTextArea((t) => {
      t.inputEl.addClass("crl-textarea");
      t.inputEl.rows = 8;
      t.setPlaceholder("Example implementation...").onChange((v) => this.example = v);
    });
    new import_obsidian.Setting(contentEl).addButton(
      (b) => b.setButtonText("Save Pattern").setCta().onClick(() => this.save())
    );
  }
  async save() {
    if (!this.title.trim()) {
    ...
    const langList = this.languages.split(",").map((l) => l.trim()).filter(Boolean);
    const date = new Date().toISOString().split("T")[0];
    const lines = [
      "---",
      `title: "${this.title}"`,
    ...
    const path = `${folder}/${safeName}.md`;
    if (this.app.vault.getAbstractFileByPath(path)) {
      new import_obsidian.Notice("A pattern with that name already exists.");
      return;
    }
    await this.app.vault.create(path, lines.join("\n"));
    new import_obsidian.Notice(`Pattern "${this.title}" saved.`);
    this.close();
    this.onSave();
  }
  onClose() {
...
var SearchTagModal = class extends import_obsidian.Modal {
  ...
  onOpen() {
    ...
    let tagQuery = "";
    new import_obsidian.Setting(contentEl).setName("Tag").addText((t) => {
    ...
    new import_obsidian.Setting(contentEl).addButton(
      (b) => b.setButtonText("Search").setCta().onClick(() => {
        ...
        results.empty();
        if (!tagQuery) {
          results.createEl("p", { text: "Enter a tag to search." });
          return;
        }
        const allFolders = [
        ...
        for (const file of this.app.vault.getMarkdownFiles()) {
          if (!allFolders.some((f) => file.path.startsWith(f + "/")))
            continue;
          const cache = this.app.metadataCache.getFileCache(file);
          const fm = (_a = cache == null ? void 0 : cache.frontmatter) != null ? _a : {};
          const tags = Array.isArray(fm["tags"]) ? fm["tags"] : typeof fm["tags"] === "string" ? [fm["tags"]] : [];
          if (tags.some((t) => t.toLowerCase().includes(tagQuery))) {
            matches.push(file);
          }
        }
        if (matches.length === 0) {
...
var CodeReferenceSettingTab = class extends import_obsidian.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.plugin = plugin;
  }
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Snippets folder").setDesc("Folder where code snippets are stored.").addText(
      (t) => t.setValue(this.plugin.settings.snippetsFolder).onChange(async (v) => {
        this.plugin.settings.snippetsFolder = v;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Patterns folder").setDesc("Folder where code patterns are stored.").addText(
      (t) => t.setValue(this.plugin.settings.patternsFolder).onChange(async (v) => {
...
var CodeReferencePlugin = class extends import_obsidian.Plugin {
  constructor() {
    super(...arguments);
  ...
  refreshSidebar() {
    this.app.workspace.getLeavesOfType(VIEW_TYPE_CODE_REF).forEach((leaf) => {
      if (leaf.view instanceof CodeRefView)
        leaf.view.render();
    });
  }
  async loadSettings() {
